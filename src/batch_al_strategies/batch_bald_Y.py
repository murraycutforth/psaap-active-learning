...
from batch_al_strategies.joint_entropy import DynamicJointEntropy
...
class CandidateBatch:
    scores_lf: list[float]
    indices_lf: list[int]
    scores_hf: list[float]
    indices_hf: list[int]
        ...
        while True:
            ...
            if flags[1] and len(X_hf_cand_pool) > 0:
                if len(selected_indices_HF) > 0:
                    # TODO: compute the distance to the current batch
                    ...
                    # TODO: consider also the distance to the current batch, or other metrics? and Gaussian bandwidth?
