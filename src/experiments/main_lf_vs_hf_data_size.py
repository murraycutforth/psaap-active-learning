...
from src.toy_example import create_smooth_change_nonlinear, create_smooth_change_linear
...
def main():
    ...
    N_H_values = [4, 8, 16, 32, 64, 128, 256, 512, 1024]
    ...
    for i, n_l in enumerate(N_L_values):
        for j, n_h in enumerate(N_H_values):
            ...
            for k in range(N_reps):
                ...
                model = BFGPC_ELBO(l2_reg_lambda=l2_reg)
    ...
    plot_heatmap(elpps, "ELPP",
             ...
             figname=output_dir / f"elpp_epochs{n_epochs}_l2{l2_reg}.png",
             val_fmt="{x:.3f}")
